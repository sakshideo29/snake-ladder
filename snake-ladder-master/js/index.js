...
function checkWinner(gamer) {
    if (gamer.currentPosi >= 100) {
        speak(gamers[curr].name + " is the Winner! Congratulations!");
        return true
    }
    return false;
}
...
function updateCurrentPlayer(index) {
    ...
    speak("It's " + gamers[index].name + "'s turn.");
...
function updateBoard(curr) {
    ...
    if (gamers[curr].currentPosi >= 100) {
        ...
        speak(gamers[curr].name + " climbed the ladder!");
        ...
        speak(gamers[curr].name + " moved to " + gamers[curr].currentPosi);
        ...
        speak(gamers[curr].name + " got bitten by a snake!");
        gamers[curr].currentPosi = newLand;
        boxid = 'box' + (gamers[curr].currentPosi)
...
function getAnswer() {
    ...
    if (mcq) {
        ...
        if (mcq.value == questions[currentQuestion].answer) {
            ...
            speak("Correct Answer. You can roll the dice.");
            ...
            speak("Wrong Answer. Next player's turn.");
